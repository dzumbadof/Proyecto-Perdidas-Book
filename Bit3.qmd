...
9.  **Nombre de Resultado**: Medidas AIC y BIC para la frecuencia
    ...
    **Resumen en un párrafo**: En la @tbl-metricasFrecuencia se
    ...
    |     versolimiltud             |     para el ajuste de la      |
    |                               |     frecuencia                |
    | -   Teoría B: Selección de    |                               |
    |     modelos con pruebas de    | -   Hipótesis B:              |
    |     bondad de ajsute          |     Distribuciones compuestas |
    |                               |     podrían dar mejores       |
    | -   Teoría C: Selección de    |     resultados en el ajuste   |
    |     modelos con el criterio   |     de la frecuencia al ser   |
    ser |     de información de Akaike  |    flexibles(se v aloran |
    |     infor(AIC) y el criterio de     por ejemplo las              |
 |     y el información bayesiano      distribuciones compuestas    |
 |     bayes(BIC)                |     Poisson-Poisson o Neyman     |
 |                               |     tipo A; Poiss                   | -   Resultado A: Ninguna de las     Polya-Aisson-Geométrica o |
    |     distrlas ibuciones de las clase Poissonlya-Aeppli; y            |     (a,b,clases 0) o (a,b,1) son   |     Poisson-Gaussiana    |     
    |
  |  |  son adecuadas para        |     inversa)                  |
    |     modelar la frecuencia de  |                               |
    |     los reclamos.         s    |
    |     frecuencia de los ramos.  |                               |
    |                               |-   Hipótesis A: La, la        |
    | -   Hipótesis A: La disbución |   iva es la que logística     |
    |     logística generalda es la |    na el mejor ajuste más     |
   que  |     más adecauda pelar la |    os datos de frecuencia     |
   ajuste  |     severidaeclamos    |                               |
   frecuencia-----------------------+------------------------------+    : Elementos de reporte

    +---------------+-----------------------------------------------------+
    | Sección       | Temasatar                                      |
    +:=============:+:==============================================:+
    | Int roducción | 1.  ducción al modelado de pérdidas.           |
    |               |Contextualización de la problemática surgida    |
    |               |  r los daños a la propiedad y a lsurgida as personas   |
    |            aeopuertos de Estados Unidos. las personas               |
    |         |
    |               |     Unidos.               | 3.  Teoría          de estimación paramétrica,pruebas de     |
    |               |    ad de ajuste y pérdidas agregade das.          |
    |               | ltados de estudios afines.                  |
    +---------------+------------------------------------------------+
    | Me todología  | 1.  Iucción de la base de datos y análisis     |
    |                 escriptivo                       análisis               |
    |                                                  |
    |               | 2.  Método A: Estimación paramétrica vía máxima     |
    |               | erosimilitud.                    máxima               |
    |                                                    |
    |               | 3.  Método B: Selección de modelos vía : AIC, BIC y |
    |               |     as Chi-Cuadrado, Kolmogorov-SBIC y mirnov y      |
    |          Anderson-Darling para modelos obteKolmogorov-Smirnov y nidos por     |
    |                              odelos obtenidos por método   |                                                |
    |               |     A.                                              |
    |               |                                           |
    |               | 4.  Método C: Método de recursión (Fórmula de       |
    |               |     P) para los modelos seleccionados según    |
    |               |  todo B.                         según               |
    +---------------+------------------------------------------+
    | R esultados   | 1.  Rado A:                                    |
    |               |                                                |
    |               | 2.  Rado B:                                    |
    |               |                                                |
    |               | 3.  Lciones de la metodología y los datos      |
    |               |     (ente)                                     |
    |               |                                           |
    |               | 4.  Recomendaciones (pendiente)                     |
    +---------------+------------------------------------------------+

Table: Distribución de conos por sección.
