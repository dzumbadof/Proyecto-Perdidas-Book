...
temp <- c(modelo$distname, round(modelo$aic,2), round(modelo$bic, 2), round(valor_p, 4))
...
tabla$`Valor $p$`[6] <- round(gofMezclaPoisson2$chisqpvalue, 4)
tabla$`Valor $p$`[7] <- round(gofMezclaPoisson3$chisqpvalue, 4)
tabla$`Valor $p$`[8] <- ""
tabla$`Valor $p$`[9] <- ""
...
```{r}
library(xtable)
print(xtable(tabla, caption = "Título de la tabla", align = "lllll" , label = "etiqueta", digits = 2, include.rownames = FALSE, include.colnames = T, format.args = list(big.mark = c(","), decimal.mark = ".")))
...
pdf(file = "Grafico ajustes mezclas.pdf",
    width = w, 
    height = h) 
par(mar = c(5,7,0.5,0.5), cex.lab=2, cex.axis=2, cex.main=2, cex.sub=2, cex.leg=2)
cdfcomp(list(fitMezclaPoisson4, fitMezclaPoisson6), main="", xlab="Datos de frecuencia", ylab = "Función de distribución", legend=c( "Mezcla 4 Poisson", "Mezcla 6 Poisson"), datacol = c("black"), fitcol = c("blue", "red"), fitlty = 1, xlegend = "topleft", cex=2)
dev.off()
...
Para esto se propone el ajuste vía máxima verosimilitud tanto de la severidad mediante distribuciones continuas, como de las frecuencias mediante distribuciones discretas tradicionales y adicionalmente mediante los métodos alternativos antes mencionados. Por último,  se realiza la selección de modelos con pruebas de bondad de ajuste y los criterios de información de Akaike (AIC) y bayesiano (BIC).
...
Entre los métodos para la estimación de parámetros de las variables de frecuencia y severidad  se encuentran el método de ajuste paramétrico vía máxima verosimilitud. Tal como lo establece @klugman2019loss Defina $n$ el tamaño de una muestra aleatoria de la variable aleatoria $X$, la cual denotamos como $(x_{1},..., x_{n})$. Si la pdf de la variable aleatoria $X$ se define como $f (x_i; \theta)$ se define por tanto la log verosimilitud de la muestra como:

$$\log L(\theta ; \boldsymbol{x})=\sum_{i=1}^{n} \log f\left(x_{i} ; \theta\right)$$

El valor de $\theta$ que maximiza la expresión anterior es el estimador de máxima verosimilitud (MLE) de  $\theta$ y es denotado por $\hat{\theta}$.

En ocasiones, los métodos paramétricos tradicionales como el mencionado anteriormente, no son suficientes para lograr un ajuste adecuado de los parámetros de una distribución, nuestro proyecto no estuvo exento de esta problemática al ajustar la frecuencia. Es por esta razón que fue necesario el uso de mixturas (o mezclas). 
...
para algunos pesos $\alpha_{j}$ que cumplen con ser positivos y sumar la unidad, y donde $\lambda_{j}>0$ para $j=1,\dots, d$. De este último hecho, se desprende que solamente es necesario estimar $d-1$ pesos, además de los $d$ parámetros $\lambda_{j}$, sumando un total de $2d-1$ parámetros a estimar.

Cabe mencionar que para el ajuste de este tipo de mixturas se optó por seguir la reparametrización presente en @zucchini2009hidden, es decir $\eta_{j} =\ln (\lambda_{j})$ para $j=1,\dots, d$ y $\tau_{j}=\frac{\alpha_{j}}{1-\sum_{i=2}^{d}\alpha_{i}}$ para $j=2,\dots, d$. Esta permite maximizar la verosimilitud sin tener que contemplar las restricciones sobre los parámetros. Naturalmente, los parámetros originales se pueden recuperar como $\lambda_{j}=e^{\eta_{j} }$, $\alpha_{j}=\frac{e^{\eta_{j}}}{1+\sum_{i=2}^{d}e^{\eta_{i}}}$  y $\alpha_{1}=1-\sum_{i=2}^{d}\alpha_{i}$.



Los análogos discretos de distribuciones continuas utilizadas en este trabajo corresponden a las distribuciones de la parte entera de distribuciones, esto es $\left \lfloor X\right \rfloor$ donde $X$ es una variable aleatoria continua.

Las pruebas de bondad de ajuste son utilizadas de forma recurrente una vez ajustadas las distribuciones a los datos y estimados los parámetros respectivos con la finalidad de elegir entre las distribuciones que compiten entre sí. Estas pruebas adoptan la estructura de prueba de hipótesis donde la hipótesis nula consiste en que los datos siguen una distribución particular.Para este estudio se implementan las pruebas:Prueba de Kolmogorov-Smirnov, prueba Anderson-Darling y la prueba Chi-cuadrado. La hipótesis nula de estas tres pruebas consiste en que $F$ es la distribución verdadera de los datos. En lo sucesivo, $F_{n}$ denota la distribución empírica.

Tal como se esteblece en @klugman2019loss si se define como $t$ el punto de truncamiento izquierdo($t=0$, en caso de no truncamiento) y u el punto de censura ($u= \infty$, en caso de no existir censura). La prueba de Kolmogorov Smirnov para datos individuales viene dada por:

$$D=\max(\vert F_{n}(x) - F(x)\vert, \vert F_{n}(x^{-}) - F(x)\vert)$$

Este valor se compara con un valor crítico de $\hat{k}$ con valores $\frac{1.22}{\sqrt{n}}, \frac{1.36}{\sqrt{n}}, \frac{1.63}{\sqrt{n}}$ para niveles de significancia dados por 0.01, 0,05 y 0,01 respectivamente, y donde se rechaza la hipótesis nula si $D>\hat{k}$.

La prueba Anderson-Darling se define como:


$$A^{2}=n \int_{t}^{u} \frac{\left[F_{n}(x)-F^{*}(x)\right]^{2}}{F^{*}(x)\left[1-F^{*}(x)\right]} f^{*}(x) d x$$

Es decir,es un promedio ponderado de las diferencias al cuadrado entre la distribución del modelo y el empírico. Y los valores críticos para esta prueba son  de 1.933, 2.492 y 3.857 para $10\%$, $5\%$ y $1\%$ niveles de significancia, respectivamente.

Como se expone en @klugman2019loss, una prueba chi-cuadrado de bondad de ajuste requiere datos agrupados en intervalos $[c_{j-1}, c_{j})$, $j=1,\dots, k$. Se define  $\hat{p}_j=F\left(c_j\right)-F\left(c_{j-1}\right)$ y $p_{n j}=F_n\left(c_j\right)-F_n\left(c_{j-1}\right)$. Luego se tienen $E_{j}=n\hat{p}_j$ y $O_{j}=p_{n j} n$ con las cantidades esperada y observada, respectivamente, de observaciones en el intervalor respectivamente. El estadístico de la prueba es $\chi^{2}=\frac{\sum_{j=1}^{k}(E_{j}-O_{j})^2}{E_{j}}$, el cual, bajo la hipótesis nula, se distribuye como una chi-cuadrado con grados de libertad $k-1-\textit{cantidad de parámetros del modelo}$.


Para la obtención del modelo de pérdidas agregadas se procede a realizarlo mediante simulación utilizando el siguiente algoritmo presentado en @klugman2019loss: 

Se simula un número de la frecuencia escogida que se llama $n$. Este es el número de eventos de pérdidas, seguido se toma una muestra de tamaño $n$ de la severidad escogida y se calcula la suma. Esta es la pérdida simulada de un mes. Finalemente, se repite el paso uno y dos hasta obtener el tamaño de muestra deseado, en este caso de 1000.

Siguiendo este algoritmo se puede construir la pérdida simulada de 1000 meses como composición de la frecuencia y la severidad escogidas. 
...
Respecto de el ajuste de la frecuencia, este resultó muy problemático. Se empezó por probar con las distribuciones discretas de la clase $(a,b,0)$ y $(a,b,1)$.

En la @tbl-metricasFrecuencia se presenta el valor $p$ resultante de la prueba chi-cuadrado de bondad de ajuste. Se obtuvieron valores $p$ muy bajos en todos los casos, con lo que se rechaza la hipótesis de bondad de ajuste bajo los niveles de significancia usuales del $10\%$, $5\%$ y $1\%$, de modo que hay evidencia suficiente para rechazar la hipótesis de que la distribución de la frecuencia proviene de cualquiera de las propuestas.

Posteriormente y buscando una mayor flexibilidad que permitiera conseguir un mejor ajuste, se se probó con las composiciones Poisson-Gaussiana inversa, Poisson-Geométrica (Distribución Polya-Aeppli) y Binomial negativa-Poisson (Delaporte). Sin embargo, nuevamente el ajuste no es adecuado. La flexibilidad que se perseguía al incorporar distribuciones más complejas no se refleja en un buen ajuste, como puede verse en la @tbl-MetricasNuevasBit4, donde los valores $p$ para estas distribuciones son muy bajos, de modo que debieron explorarse otras alternativas. Cabe mencionar que de las tres distribuciones, la Polya-Aeppli mereció un mayor valor $p$ en la prueba chi-cuadrado, sin llegar a ser este significativo, y mejores valores de AIC y BIC.

Luego, se pensó que versiones discretas de distribuciones continuas podrían ser usadas para la modelización de la frecuencia. En este análisis nos centramos en las variaciones disponibles en el paquete \texttt{extraDistr} de \texttt{R}: la distribución Weibull discreta y la Gamma discreta. En este contexto, la discretización corresponde a que es la distribución de la parte entera de una variable aleatoria absolutamente continua. De la misma @tbl-MetricasNuevasBit4 se ve que estas opciones resultan las mejores opciones hasta el momento, sin embargo, no puede dejar de notarse que los valores $p$ son muy bajos, rechazándose así la hipótesis de bondad de ajuste bajo los niveles de significancia usuales. 

Como un último intento de ajuste de la frecuencia y motivado por la forma del histograma en la Figura @fig-histograma_frecuencia surge la idea de probar con mezclas de distribuciones discretas, limitadas al caso de mezclas de tipo Poisson, probándose con mezclas dos, tres, cuatro y seis de estas. Visualmente y como puede corroborarse de las figuras [-@fig-ajusteMezclasPoisson23] y [-@fig-ajusteMezclasPoisson46], el ajuste que proporcionan las mezclas a partir de tres es muy superior al cualquier otro enfoque emprendido anteriormente. En términos de valor $p$ en la prueba chi-cuadrado, sin embargo, de la tabla @tbl-MetricasNuevasBit4 puede notarse que para la mezcla de tres Poissones el resultado no es bueno. 

Además, obsérvese que en la tabla @tbl-MetricasNuevasBit4 no se muestran valores $p$ para los dos ajsutes que mejor lucen. Recuérdese que las mezclas tienen un nivel de complejidad creciente porque cada vez se requiere estimar una mayor cantidad de parámetros, para lo cual no se tiene una gran cantidad de datos (solo 48). Este hecho imposibilita una adecuada utilización de la prueba chi-cuadrado, ya que, con los datos de frecuencia de este trabajo, una cantidad que oscila entre los siete y ocho intervalos se requieren para garantizar que las frecuencias esperadas en cada intervalo sean iguales o superiores a cinco, lo cual constituye una regla empírica para el correcto funcionamiento de esta prueba estadística. De esta manera, con mezclas de cuatro y seis distribuciones, se tienen que estimar siete y once parámetros respectivamente, con lo cual, en la prueba chi-cuadrado de bondad de ajuste se tendrían grados de libertad negativos. En resumen, no se puede cuantificar la evidencia a favor de la bondad de ajuste de estos dos modelos más complejos con la prueba planteada y solamente se aprecia esta de forma gráfica. Adicionalmente, se prevee que se está cayendo es sobreajuste. Dicho esto, los criterios de información ubican a la mezcla de cuatro y seis distribuciones Poisson por encima de la de tres. 

Aunque no se puede llevar a cabo el proceso de selección de modelos planteado en un inicio, con la prueba chi-cuadrado como un primer proceso de eliminación y luego con el AIC y BIC para determinar el mejor modelo, se decide retener para la modelización de la frecuencia las mezclas de distribuciones Poisson, que gráficamente resultaron las mejores. 
...
#| echo: false
#| fig-cap: "Función cumulativa de las distribuciones ajustadas con MLE"
#| label: fig-cdfcompSeveridad1
cdfcomp(cex=0.8, list(fit_lnorm, fit_exp, fit_gamma, fit_JSU, fit_glogis, 
                      fit_weibull), main = "", xlab = 'Datos de severidad',
        ylab = 'Función de distribución', legendtext = plot.legend)
```

La @fig-denscompSeveridad1 muestra las densidades estimadas contra los datos observados. Se puede apreciar que todas siguen una forma similar excepto por la exponencial, que muestra indicios de mal ajuste. La Weibull parece tener una medida de centralidad más alta que el resto, mientas que la logística muestra evidencia de tener una centralidad más baja. Sin embargo, todas aproximan bastante bien la distribución de los datos. @fig-cdfcompSeveridad1 muestra la función de distribución estimada contra la distribución empírica de los datos. Muestra evidencia que apoya lo que se había observado y todas las distribuciones candidatas muestran buen ajuste, al menos visualmente, y parecen capturar bien la distribución real de los datos, excepto por la exponencial que diverge significativamente del resto. 
...
@tbl-metricasSeveridad muestra los resultados de la pruebas de bondad de ajuste. Para la prueba de Kolmogorov Smirnov, una significancia del 5% corresponde a un valor crítico de 0.196299, lo que excluye la exponencial, pero todas las demás son candidatas viables. Para la prueba de Anderson-Darling, una significancia del 5% corresponde a un valor crítico de 2.492 lo que nuevamente rechaza la exponencial rotundamente pero deja todas las demás distribuciones como viables. Al observar el AIC y BIC vemos que la gamma proporciona el mejor ajuste con la log-normal muy cercana también. Bajo estas medidas se decide elegir la gamma como la distribución de la severidad, sin embargo cabe mencionar que la log-normal muestra suficiente evidencia de bondad de ajuste y se pudo haber escogido también.


###### Composición de frecuencia y severidad
...
La @fig-DistS1 muestra las distribución de las pérdidas simuladas con cada mixtura de Poisson. Se nota consistencia entre ellos, sin embargo la forma parece ser distinta a la de los datos originales, más específicamente, parecen estar centradas más a la derecha. A pesar de que el promedio de las pérdidas simuladas y las originales es muy parecido, el VaR calculado con las simulaciones es más bajo que el empírico. Esto podría explicarse del hecho de que los datos originales muestran ocurrencias de pérdias anormalmente altas, como lo es el mes de enero del 2012. Una vez que se obtuvo la composición de la pérdidas simuladas se puede calcular el VaR al 95%. Se observa en @tbl-VaR1 que el VaR empírico con los datos reales es más alto que el VaR calculado con las simulaciones utilizando todas las mixturas de Poisson propuestas. Esto podría explicarse por el mal ajuste residual de la frecuencia y podría causar problemas de subestimación de las pérdidas. 
